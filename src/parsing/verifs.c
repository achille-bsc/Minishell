...
/*   Updated: 2025/05/27 00:18:03 by abosc            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	verif(t_mnours *mnours)
{
	if (check_quotes(1, mnours->line) == 1)
		return (ft_error("Syntax Error: incorrect quotes (single)", mnours),
			mnours->exit = 1, 1);
	if (check_quotes(2, mnours->line) == 1)
		return (ft_error("Syntax Error: incorrect quotes (double)", mnours),
			mnours->exit = 1, 2);
	return (0);
}
